...
class CamSimulationslauf(models.Model):
    """Monte-Carlo-Simulationslauf eines CAM-Szenarios.

    Analog zu ``apps.berechnung.models.Simulationslauf`` (eigene Status-
    Choices statt App-übergreifendem Import, damit ``apps.cam`` unabhängig
    bleibt). Die Berechnung läuft asynchron in einem Hintergrund-Thread
    (siehe ``services.py``) und schreibt Status/Ergebnis hierher.
    """
    ...
    szenario = models.ForeignKey(
        CamSzenario, related_name="laeufe", on_delete=models.CASCADE, verbose_name="CAM-Szenario"
    )
    ...
    class Meta:
        verbose_name = "CAM-Simulationslauf"
        verbose_name_plural = "CAM-Simulationsläufe"
